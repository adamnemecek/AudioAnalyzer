...
class Meter {
    ...
    func initialize() {
        ...
        clipHolder.setTime(1.0, sampleRate: fs)
    ...
    func getCurrentValues() -> Dynamics {
		let values = Dynamics(name: ID,
                              peakValue: dbManager.mag2dB(mag: peak.value),
                              rmsValue: dbManager.mag2dB(mag: rms.value),
                          	  vuValue: dbManager.mag2dB(mag: vu.value),
                              peakMax: peak.holder.value,
                              rmsMax: rms.holder.value,
                              ispMax: ispHolder.value,
